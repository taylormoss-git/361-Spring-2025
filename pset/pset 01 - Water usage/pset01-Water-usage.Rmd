...
for(s in sources){
  ...
  for(year in years){
    ...
    ## read in the raw data into a dataframe
    ...
    df = df %>%
      ...
      separate(col = crop, into = c('src', 'crop', 'year'), sep = '[.]', fill = 'right') %>%
    ...
    ## add the df we just made to our aggregate dataframe d
    d = rbind(d, df)
  }
...
Find the **annual** mean **water usage**, the change from 2008 to 2020, and the percent change from 2008 to 2020, for each crop and each source (SWW, GWW, GWD). 
...
dd3 = dd2 %>%
  ...
  relocate(crop, src, mean, change, percent_change)
...
g = ggplot(dd3, 
  ...
  geom_col(width = 0.8, 
           position = position_dodge(width = 0.8)) + 
  labs(title = "Percent Change in Water Usage by Crop and Source",
...
title = "Average irragation water usage by crop and source"
g = ggplot(dd3, 
           aes(x    = crop, 
               y    = src, 
               fill = mean, 
  ...
  geom_tile(linewidth   = 0.4, 
            show.legend = T, 
            color       = pubdarkgray) +
  scale_fill_gradient(low      = publightgray,
                      high     = pubblue, 
                      na.value = pubdarkgray, ## same color as below
                      oob      = squish 
                      ) +
  ...
  guides(alpha = guide_legend(
    title = "No data",
    override.aes = list(fill = pubdarkgray))) + ## same color as above
...
dd3 %>% 
  ggplot() + 
  geom_mosaic(aes(x = product(crop), 
                  fill = src, 
                  weight = mean)) + 
  theme_mosaic()
...
Some of the benefits of these two plots are that 
