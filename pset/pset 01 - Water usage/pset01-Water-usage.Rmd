...
unzip(zipfile = 'rawdata/DOI-10-13012-b2idb-4607538_v1.zip', 
      list = TRUE) ## this lists the filenames, but does not unzip the file
```
...
for(s in sources){
  cat(s, '') ## set up columns (gwd, sw, and gwa) i.e. loop through the sources
  
  for(year in years){
    cat(year, '') ## set up row headers for each year i.e. loop through years
...
```{r}
# overall national totals summed across GeoID's (locations), for each crop, source, and year
dd1 = d %>%
  group_by(crop, src, year) %>%
  summarize(value = sum(value))
dd1

# mean across years (annual mean)
dd2 = dd1 %>%
  group_by(crop, src) %>%
