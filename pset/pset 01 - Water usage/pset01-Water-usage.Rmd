...
```{r}
## Change Crop Names to common names
dd2$crop = recode(dd2$crop,
                  "barley" = "Barley",
                  "corn" = "Corn",
                  "cotton" = "Cotton",
                  "rice" = "Rice",
                  ...
                  "oats" = "Oats",
                  "potatoes" = "Potatoes",
                  "other_sctg2" = "Other Grains",
                  "other_sctg3" = "Other Produce",
                  "other_sctg4" = "Other Animal Feed")

## Make ggplot of mean usage data
title = "Mean Yearly Water Usage from 2008 to 2020"
g = ggplot(dd2, aes(x = reorder(crop, mean, sum), 
                    y = mean, 
                    fill = src)) +
  geom_col(position = position_dodge()) +
  labs(x = "Crop", 
       y = expression(paste("Mean Water Usage ", (km^3 / year))), 
       title = title,
       fill = "Source:") +
  theme_pub() +
  ...
  scale_fill_discrete(labels = c("Ground Withdrawals",
                                 "Ground Depletions",
                                 "Surface Withdrawals"))
...
## Make ggplot of change data
title = "Change in Yearly Water Usage from 2008 to 2020"
g = ggplot(dd2, aes(x = reorder(crop, change, sum), 
                    y = change, 
                    fill = src)) +
  geom_col(position = position_dodge()) +
  labs(x = "Crop", 
       y = expression(paste("Change in Water Usage ", (km^3))), 
       title = title,
       fill = "Source:") +
  theme_pub() +
  theme(axis.text.x = element_text(angle = 45, 
...
## Make ggplot of percent change data
title = "Percent Change in Yearly Water Usage from 2008 to 2020"
g = ggplot(dd2, aes(x = reorder(crop, perc_change, sum), 
                    y = perc_change, 
                    fill = src)) +
  geom_col(position = position_dodge()) +
  labs(x = "Crop", 
